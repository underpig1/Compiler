...
def enter():
    global challenge, notebook, text, var, var0, var1, var2, var3, var4, var5, check0, check1, check2, check3, check4, check5, tab0, tab1
    challenge += 1
    if challenge == 0:
        ...
        example.config(text = "")
        os.makedirs(os.path.dirname(os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/src/src"), exist_ok = True)
        with open(os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/README.md", "w+") as file:
            file.write(text.get("1.0", tk.END))
            file.close()
        ssl._create_default_https_context = ssl._create_unverified_context
        urllib.request.urlretrieve("https://raw.githubusercontent.com/underpig1/Compiler/master/requests/ide.py", os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/src/ide.py")
        urllib.request.urlretrieve("https://raw.githubusercontent.com/underpig1/Compiler/master/requests/launch.command", os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/launch.command")
        info.update({"tag":"." + info["name"][:-1][:3].lower()})
        open(os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/src/info.txt", "w+").write(str(info))
        os.system("cd " + os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1])
        os.system("chmod +x " + os.path.expanduser("~/Desktop/Compile_") + info["name"][:-1] + "/launch.command")
    elif challenge == 91:
